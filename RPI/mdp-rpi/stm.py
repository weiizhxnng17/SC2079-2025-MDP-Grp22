...
class STMInterface:
        ...
        while True:
            ...
            if message_type == "NAVIGATION":
                ...
                for idx, command in enumerate(commands):
                    if not any(x in message["data"]["commands"] for x in ["SECONDLEFT", "SECONDRIGHT"]):
                        if idx >= len(commands) - NUM_IMAGES:
                            # Capture and send image to Android
                            capture_and_send_image_thread = threading.Thread(
                                target=self.send_image_to_android,
                                kwargs={"final_image": False},
                                daemon=True
                            )
    ...
    def send_image_to_android(self, final_image=False):
        # Capture image and send to Android
        try:
            image_message = get_image(final_image=final_image)
            self.RPiMain.Android.msg_queue.put(image_message)
            print("[STM] Sent image to Android, final_image =", final_image)
        except Exception as e:
            print("[STM] ERROR: Failed to capture or send image -", str(e))
