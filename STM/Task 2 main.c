...
volatile float ultrasonic_measure = 0;
volatile int ir_left = 0;
volatile int ir_right = 0;
volatile int ir_start = 0;
volatile float avg_us = 0.0f;
volatile uint8_t  us_avg_ready = 0;   // set when avg_us is freshly computed
...
#define TURN_PWM_MAX           5250    // your existing run level
...
volatile uint8_t  us_wait_fall = 0, us_ready = 0;
/* USER CODE END PV */
...
/* USER CODE BEGIN PD */
// Ultrasonic timing
#define US_TRIG_TIM        htim3
#define US_TRIG_CH         TIM_CHANNEL_1   // PC6

#define US_ECHO_TIM        htim3
#define US_ECHO_CH         TIM_CHANNEL_2   // PC7



// TIM3 runs at 1 MHz (1 µs per tick) -> Prescaler = 71 with 72 MHz APB1 timer clock
#define US_TIM_PRESCALER      71
#define US_TIM_PERIOD         19999           // 20 ms frame (safe, HC-SR04 needs >60 ms)
#define US_TRIG_PULSE_TICKS   10
...
/* --- Command Cooldown --- */
#define CMD_COOLDOWN_MS   10u   // tweak 150–500 ms as you like
...
static void MX_TIM3_Init(void)
{
  ...
  htim3.Instance = TIM3;
  htim3.Init.Prescaler         = 71;        // 1 µs tick
  htim3.Init.CounterMode       = TIM_COUNTERMODE_UP;
  htim3.Init.Period            = 59999;     // 20 ms
  ...
  sConfigOC.OCMode     = TIM_OCMODE_PWM1;
  sConfigOC.Pulse      = 10;   // 10 µs
  sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
  HAL_TIM_PWM_ConfigChannel(&htim3, &sConfigOC, TIM_CHANNEL_1);

  // Input capture on CH2 (ECHO)
...
static void CommandQueue_TryDispatch(void)
{
  ...
  char c2 = (char)toupper((unsigned char)s[2]);
  const char *p = s + 2;
  while (*p == ' ' || *p == '\t') p++;

  // FR/FL = 90° proper align
  if (c0=='L' && isdigit((unsigned char)s[1])) {
    int deg = atoi(&s[1]);
  ...
  if (c0=='R'&& isdigit((unsigned char)s[1])) {
    int deg = atoi(&s[1]);
  ...
  if (c0 == 'Y' && c1 == 'F') {
      ultrasonic_measure = 1;
      us_avg_ready = 0;

      uint32_t t0 = HAL_GetTick();
      while (!us_avg_ready && (HAL_GetTick() - t0) < 1500) {
          osDelay(5);
      }
      if (!us_avg_ready) { uart3_send("ERR YF NOECHO\r\n"); return; }

      float d = avg_us;  // cm
      if (d > 400.0f || d < 5.0f)
      {
    	  //
    	  StartMoveCM(25,DIR_BACK);
    	  uart3_send("YF5\r\n");
    	  return;
      }
      ...
      if (d <= 30.0f) {
          // 1) back up 25 cm
    	  int goback= (int)(d-30.f);
    	  goback=fabs(goback);
          StartMoveCM(goback, DIR_BACK);

          // 2) then send (d + 25) as an integer
          int after_move = (int)(d + 0.5f);


          char line[16];
          int n = snprintf(line, sizeof(line), "YF%d\r\n", after_move);
          if (n > 0 && n < (int)sizeof(line)) {
              HAL_UART_Transmit(&huart3, (uint8_t*)line, (uint16_t)n, HAL_MAX_DELAY);
          }
          return;
      }

      // otherwise, proceed with the forward move logic:
      float to_go = d - 32.0f;  // stop ~30 cm with 2 cm margin
      if (to_go < 0.0f) to_go = 0.0f;

      // send just the measured distance as integer
      {
          char line[16];
          int distance_cm = (int)(d + 0.5f);
          int n = snprintf(line, sizeof(line), "YF%d\r\n", distance_cm);
          if (n > 0 && n < (int)sizeof(line)) {
              //uart3_send(line);
        	  HAL_UART_Transmit(&huart3, (uint8_t*)line, (uint16_t)n, HAL_MAX_DELAY);
      ...
      if (to_go >= 10.0f) StartMoveCM((int)(to_go + 0.5f), DIR_FWD);
      else uart3_send("YF30\r\n");
      return;
  }
  ...
  if (c0=='F' && c1=='L' && isdigit((unsigned char)s[2])) { int deg=atoi(&s[2]); if (deg<=0){uart3_send("ERR L0\r\n");return;} if (Servo_RequestBangBangTurn(+ (float)deg,4500)==0) return; }
  if (c0=='F' && c1=='R' && isdigit((unsigned char)s[2])) { int deg=atoi(&s[2]); if (deg<=0){uart3_send("ERR R0\r\n");return;} if (Servo_RequestBangBangTurn(- (float)deg,4500)==0) return; }

  // ABS
  if (c0=='A' && c1=='B' && toupper((unsigned char)s[2])=='S') { int absd=atoi(&s[3])%360; if (Servo_RequestTurnTo((float)absd)==0) uart3_send("A\r\n"); else uart3_send("BUSY\r\n"); return; }

  // Distance FW/BW
  if (c0=='F' && c1=='W') { int cm=0; if (sscanf(p,"%d",&cm)==1 && cm>0){ StartMoveCM(cm, DIR_FWD); char b[32]; int n=snprintf(b,sizeof b,"A\r\n"); } else uart3_send("ERR (use FW###)\r\n"); return; }
  if (c0=='B' && c1=='W') { int cm=0; if (sscanf(p,"%d",&cm)==1 && cm>0){ StartMoveCM(cm, DIR_BACK); char b[32]; int n=snprintf(b,sizeof b,"A\r\n"); } else uart3_send("ERR (use BW###)\r\n"); return; }
...
void distance(void *argument)
{
  ...
  for (;;)
  {
    ...
    if (motionActive && avg >= (float)targetdistance_cm && !ir_left && !ir_right)
    {
      AllStop();
      motionActive       = 0;
      targetdistance_cm  = 0;
...
static uint16_t adc2_read_once(void) {
    uint16_t v = 0;
...
static uint16_t median9(uint16_t *a) {
    // tiny insertion sort, N=9
    for (int i=1;i<9;i++){
        uint16_t x=a[i]; int j=i-1;
        while (j>=0 && a[j]>x){ a[j+1]=a[j]; j--; }
        a[j+1]=x;
    }
    return a[4]; // middle (0..8)
}
...
void ir(void *argument)
{
  //const char *hdr = "t_ms,L_raw,L_cm,R_raw,R_cm\r\n";
  ...
  for (;;)
  {
	  if(ir_start)
	  {
		  ResetDistanceCounts();   // zeros total_counts_* and distance_cm_* safely
		  dir=DIR_FWD;
		  ir_start=0;
	  }
	  if(ir_left)
	  {
		  uint16_t bufL[9];
		  /* Left (ADC1_CH10 / PC0) median-9 */
		  for (int i=0;i<9;i++) bufL[i] = adc1_read_once();
		  uint16_t rawL = median9(bufL);
		  int32_t dL_cm = IrConvert((int32_t)rawL);
		  osDelay(50);
		  motionActive = 1;
		  if (dL_cm >= 45 ||  dL_cm == 0)
		  {
			  AllStop();
			  taskENTER_CRITICAL();
			  ir_start_avg = (int)(fabsf(distance_cm_A) + fabsf(distance_cm_D) + 54.0f)/4;
			  taskEXIT_CRITICAL();
			  char line[16];
				int n = snprintf(line, sizeof(line), "IR%d\r\n", ir_start_avg);
				if (n > 0 && n < (int)sizeof(line)) {
						//uart3_send(line);
				 HAL_UART_Transmit(&huart3, (uint8_t*)line, (uint16_t)n, HAL_MAX_DELAY);
				}
	  ...
	  {
		  uint16_t bufR[9];
	      /* Right (ADC2_CH11 / PC1) median-9 */
	      for (int i=0;i<9;i++) bufR[i] = adc2_read_once();
	      uint16_t rawR = median9(bufR);
	      int32_t dR_cm = IrConvert((int32_t)rawR);
	      osDelay(50);
	      motionActive = 1;
	      /*
	      if(ir_right && dR_cm >= 50)
	      {
	    	  char line[16];
	    	  int n = snprintf(line, sizeof(line), "IR0\r\n");
			  if (n > 0 && n < (int)sizeof(line)) {
	    	 	    	 	//uart3_send(line);
					HAL_UART_Transmit(&huart3, (uint8_t*)line, (uint16_t)n, HAL_MAX_DELAY);
					 ir_right=0;
	      ...
	      if (dR_cm >= 45 || dR_cm == 0)
	      {
	    	  AllStop();
	    	  taskENTER_CRITICAL();
			  ir_start_avg = (int)(fabsf(distance_cm_A) + fabsf(distance_cm_D) - 8.0f)/2;
			  taskEXIT_CRITICAL();
	    	  char line[16];
	    	  int n = snprintf(line, sizeof(line), "IR%d\r\n", ir_start_avg);
	    	  if (n > 0 && n < (int)sizeof(line)) {
     ...
     /* char line[64];
      int n = snprintf(line, sizeof(line), "%lu,%u,%ld,%u,%ld\r\n",
                       (unsigned long)HAL_GetTick(),
                       (unsigned)rawL, (long)dL_cm,
                       (unsigned)rawR, (long)dR_cm);
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    uint32_t cap = HAL_TIM_ReadCapturedValue(&htim3, TIM_CHANNEL_2);
...
void ultrasonic(void *argument)
{
  ...
  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1);     // TRIG
  HAL_TIM_IC_Start_IT(&htim3, TIM_CHANNEL_2);   // ECHO

  for (;;)
  {
    if (ultrasonic_measure)
    {
      float sum = 0.0f;
      int   cnt = 0;
      ...
      for (int i = 0; i < 10; ++i)
      {
        /* wait up to ~60 ms for an echo for this sample */
        uint32_t t0 = HAL_GetTick();
        while (!us_ready && (HAL_GetTick() - t0) < 60) {
          osDelay(1);
        }
        ...
        if (us_ready) {
          ...
          uint32_t width = (us_fall >= us_rise) ? (us_fall - us_rise)
                                                : (arr + 1u - us_rise + us_fall);
          float dist_cm = width / 58.0f;  // ~cm
          sum += dist_cm;
          cnt++;
        }

        osDelay(10);  // small gap between tries
      }

      if (cnt > 0) {
        avg_us = sum / (float)cnt;
      } else {
        avg_us = 0.0f;  // indicates failure
      }

      us_avg_ready = 1;       // <-- signal "fresh average ready"
      ultrasonic_measure = 0; // clear request
    }
