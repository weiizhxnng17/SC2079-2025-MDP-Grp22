...
export default function Simulator() {
  ...
  const compute = () => {
    ...
    QueryAPI.query(obstacles, robotX, robotY, robotDir, (data, err) => {
      if (data && data.data && data.data.path && data.data.commands) {
        ...
        // Handle the case where data is invalid or incomplete
        console.error("Invalid data received from API:", data, err);
        setPath([]);
        setCommands([]);
      }
      ...
      for (let j = 0; j < 20; j++) {
        ...
        if (foundOb) {
          if (foundOb.d === Direction.WEST) {
            cells.push(
              <td key={`${i}-${j}`} className="border border-l-4 border-l-red-500 w-5 h-5 md:w-8 md:h-8 bg-blue-700" />
            );
          } else if (foundOb.d === Direction.EAST) {
            cells.push(
              <td key={`${i}-${j}`} className="border border-r-4 border-r-red-500 w-5 h-5 md:w-8 md:h-8 bg-blue-700" />
            );
          } else if (foundOb.d === Direction.NORTH) {
            cells.push(
              <td key={`${i}-${j}`} className="border border-t-4 border-t-red-500 w-5 h-5 md:w-8 md:h-8 bg-blue-700" />
            );
          } else if (foundOb.d === Direction.SOUTH) {
            cells.push(
              <td key={`${i}-${j}`} className="border border-b-4 border-b-red-500 w-5 h-5 md:w-8 md:h-8 bg-blue-700" />
            );
          } else if (foundOb.d === Direction.SKIP) {
            cells.push(
              <td key={`${i}-${j}`} className="border w-5 h-5 md:w-8 md:h-8 bg-blue-700" />
            );
          }
          ...
          if (foundRobotCell.d !== null) {
            ...
            cells.push(
              <td key={`${i}-${j}`} className="bg-green-600 border-white border w-5 h-5 md:w-8 md:h-8" />
            );
          }
        } else {
          cells.push(
            <td key={`${i}-${j}`} className="border-black border w-5 h-5 md:w-8 md:h-8" />
          );
        }
      }
